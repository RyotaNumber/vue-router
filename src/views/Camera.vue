    ...
    <v-dialog v-model="showDialog" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="secondary" @click="showDialog = false"> 閉じる </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  ...
  methods: {
    ...
    onPhotoSelected(event) {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          // 再撮影の場合は現在のインデックスの写真を上書き
          if (this.currentPhotoIndex !== null) {
            this.$set(this.photos, this.currentPhotoIndex, e.target.result);
            this.currentPhotoIndex = null; // リセット
          } else {
            this.photos.push(e.target.result); // 新しい写真を追加
          }
        };
